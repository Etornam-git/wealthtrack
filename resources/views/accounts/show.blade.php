<x-layout>
    ...
    <div class="max-w-4xl mx-auto p-6 bg-white shadow-xl rounded-2xl mt-6">
      ...
      <div class="mb-6">
        ...
        <p><strong>Created On:</strong> {{ $account->created_at->format('F d, Y') }}</p>
      </div>
      ...
      <div class="flex space-x-4 mb-6">
        ...
        <form action="/accounts" method="POST" class="inline-block hidden"
              onsubmit="return confirm('Are you sure you want to delete this account?');">
          @csrf
          @method('DELETE')
          <button type="submit"
                  class="bg-red-500 hover:bg-red-600 text-white font-semibold py-2 px-4 rounded-xl">
            Delete Account
          </button>
        </form>
      </div>
